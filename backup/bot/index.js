...
const pingpongStrategy = async (jupiter, tokenA, tokenB) => {
	...
	try {
		...
		// OPTIMIZATION 1: Add route computation settings for better performance
		const routes = await jupiter.computeRoutes({
			...
			slippage,
			forceFetch: true,
			// PERFORMANCE BOOST: These settings speed up route computation
			onlyDirectRoutes: false,  // Set to true for 30-50% faster routing (may miss opportunities)
			filterTopNResult: 3,      // Only check top 3 routes instead of all
		});
		...
		// OPTIMIZATION 2: Better route selection - choose best route based on output
...
const arbitrageStrategy = async (jupiter, tokenA) => {
	...
	try {
		...
		// OPTIMIZATION 3: Add route computation settings
		const routes = await jupiter.computeRoutes({
			inputMint: new PublicKey(inputToken.address),
			outputMint: new PublicKey(outputToken.address),
			inputAmount: amountToTrade,
			...
			forceFetch: true,
			// PERFORMANCE BOOST
			onlyDirectRoutes: false,  // Consider setting to true for speed
			filterTopNResult: 5,      // Check more routes for arbitrage (needs variety)
		});
		...
		// CRITICAL FIX: Using route index [1] instead of [0] - this might skip best route!
		// OPTIMIZATION 4: Loop through routes to find most profitable
		let bestRoute = routes.routesInfos[0];
		let bestProfit = calculateProfit(baseAmount, bestRoute.outAmount);
		
		// Check top 3 routes to find best profit (arbitrage needs this!)
		for (let routeIndex = 1; routeIndex < Math.min(3, routes.routesInfos.length); routeIndex++) {
			const testRoute = routes.routesInfos[routeIndex];
			const testProfit = calculateProfit(baseAmount, testRoute.outAmount);
