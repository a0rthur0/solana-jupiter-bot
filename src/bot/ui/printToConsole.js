    ...
    if (cache.ui.allowClear) {
      ...
      ui.div(
        ...
        },
        {
          text: `RPC: ${chalk[cache.ui.defaultColor](
            cache.ui.hideRpc
              ? `${cache.config.rpc[0].slice(0, 5)}...${cache.config.rpc[0].slice(-5)}`
              : cache.config.rpc[0]
          )}`,
        }
      );
      ...
      ui.div(
        ...
        Object.values(cache.queue)
          .map((v) => `${chalk[v === 0 ? "green" : v < 0 ? "yellow" : "red"]("●")}`)
          .join(" ")
      );
      ...
      const displayTokenFrom = tokenA;
      const displayTokenTo = tokenB;

      ui.div(
        `${
          cache.tradingEnabled ? "TRADING" : chalk.bold.magentaBright("SIMULATION")
        }: ${chalk.bold[cache.ui.defaultColor](displayTokenFrom.symbol)} ${
          cache.config.tradingStrategy === "arbitrage"
            ? ""
            : `-> ${chalk.bold[cache.ui.defaultColor](displayTokenTo.symbol)}`
        }`,
        `ROUTES: ${chalk.bold.yellowBright(
          cache.availableRoutes[cache.sideBuy ? "buy" : "sell"]
        )}`,
        `STRATEGY: ${chalk.bold[cache.ui.defaultColor](cache.config.tradingStrategy)}`,
        {
          text: statusMessage,
        }
      );
      ...
      ui.div(
        ...
        { text: " " },
        {
          text: `MIN. OUT: ${chalk.magentaBright(
            toDecimal(route.outAmountWithSlippage || 0, outputToken.decimals)
          )}`,
        },
        {
