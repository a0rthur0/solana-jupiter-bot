...
function printToConsole({
  ...
  try {
    if (cache.ui.allowClear) {
      ...
      // FIX: Calculate outAmountWithSlippage if not provided by Jupiter v6
      let minOutAmount;
      if (route.outAmountWithSlippage) {
        minOutAmount = route.outAmountWithSlippage;
      } else {
        // Calculate: outAmount * (1 - slippage/10000)
        // slippage is in basis points (50 = 0.5%)
        const slippageBps = typeof cache.config.slippage === 'number' 
          ? cache.config.slippage 
          : 50;
        const slippageFactor = 1 - (slippageBps / 10000);
        const outAmountNum = Number(route.outAmount);
        minOutAmount = String(Math.floor(outAmountNum * slippageFactor));
      }
      ...
      ui.div(
        ...
        },
        {
          text: `MIN. OUT: ${chalk.magentaBright(
            toDecimal(minOutAmount, outputToken.decimals)
          )}`,
        },
        {
